...
import { Table, Button, Modal, Form, InputNumber, Select, Input, message, Typography, Space } from 'antd'
import { PlusOutlined } from '@ant-design/icons'
import { getIncome, createIncome, getParts } from '../api'
...
export default function Income() {
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [inc, p] = await Promise.all([getIncome(), getParts()])
      setData(inc)
      setParts(p)
    } finally {
      setLoading(false)
  ...
  return (
    <>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Title level={4} style={{ margin: 0 }}>Приходы</Title>
